...
class ConvRNN2D(Layer):
    '''Abstract base class for deconvolutional recurrent layers.
...
class DeconvLSTM2D(ConvRNN2D):
    ...
    def __init__(self, nb_filter, nb_row, nb_col,
        ...
        super(DeconvLSTM2D, self).__init__(**kwargs)

    def get_output_shape_for(self, input_shape):
        # temperorily not using output_shape param
    ...
    def get_initial_states(self, X):
        ...
        initial_state = self.deconv_step(initial_state, K.zeros(self.W_shape),
    ...
    def deconv_step(self, x, W, b=None, border_mode='valid'):
        input_shape = self.input_spec[0].shape

        output_shape=self.get_output_shape_for(input_shape)[1:]
        #print (self.subsample)
        #output_shape=[None,16,64,64]
        deconv_out = K.deconv2d(x, W, output_shape,
        ...
        if b:
            if self.dim_ordering == 'th':
                deconv_out = deconv_out + K.reshape(b, (1, self.nb_filter, 1, 1))
            elif self.dim_ordering == 'tf':
                deconv_out = deconv_out + K.reshape(b, (1, 1, 1, self.nb_filter))
            else:
                raise ValueError('Invalid dim_ordering:', self.dim_ordering)
        return deconv_out
    ...
    def step(self, x, states):
        ...
        #return h_tm1,[h_tm1,c_tm1]

        x_i = self.deconv_step(x * B_W[0], self.W_i, self.b_i,
                             border_mode=self.border_mode)
        x_f = self.deconv_step(x * B_W[1], self.W_f, self.b_f,
                             border_mode=self.border_mode)
        x_c = self.deconv_step(x * B_W[2], self.W_c, self.b_c,
                             border_mode=self.border_mode)
        x_o = self.deconv_step(x * B_W[3], self.W_o, self.b_o,
                             border_mode=self.border_mode)
...
class ConvGRU2D(ConvRNN2D):
    ...
    def step(self, x, states):
        ...
        h_h = self.conv_step_hidden(h_tm1 *B_U[2]* r , self.U_h,
                                    border_mode='same')


        #hh = self.activation(x_h + K.dot(r * h_tm1 * B_U[2], self.U_h))

        h_hat=self.activation(h_h + x_h)
...
class DeconvGRU2D(ConvRNN2D):
    ...
    def __init__(self, nb_filter, nb_row, nb_col,
        ...
        super(DeconvGRU2D, self).__init__(**kwargs)
    ...
    def get_initial_states(self, X):
        ...
        # (samples,row, col, filter)
        initial_state = K.sum(initial_state, axis=1)
        initial_state = self.deconv_step(initial_state, K.zeros(self.W_shape1),
                                       border_mode=self.border_mode)
       # print(initial_state.shape)
    ...
    def step(self, x, states):
        ...
        x_z = self.deconv_step(x * B_W[0], self.W_z, self.b_z,
                             border_mode=self.border_mode)
        x_r = self.deconv_step(x * B_W[1], self.W_r, self.b_r,
                             border_mode=self.border_mode)
        x_h = self.deconv_step(x * B_W[2], self.W_h, self.b_h,
                             border_mode=self.border_mode)
